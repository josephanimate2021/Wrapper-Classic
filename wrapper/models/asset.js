		...
		switch (v.type) {
			case "char": {
				...
				if (v.subtype == "video") {
					...
					xml = `<prop subtype="0" id="${v.id}" enc_asset_id="${v.id}" name="${v.title}" enable="Y" ${v.ptype != "placeable" ? `${v.ptype}="1"` : ""} placeable="1" facing="left" width="0" height="0" asset_url="/assets/${v.id}" thumbnail="/assets/${v.id}"/>`;
				}
				break;
			} case "sound": {
